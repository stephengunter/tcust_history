...
$(function () {
    ...
    $('.drag').on("touchstart touchmove touchend", function (e) {
        ...
        for (var i = 0; i < event.targetTouches.length; i++) {
            ...
            if (touch) {//touch.pageX, touch.pageY
                ...
                } else if (event.type == 'touchmove') {//&& Math.abs(touch.pageY) > Math.abs(touch.pageX)
                    moveTimeline(touch.pageY, 'v', $(this)); // 水平 下
                } else if (event.type == 'touchend') {
                    isLockPanrightPanLeft = false;
                } else if (event.type == 'touchmove') {// && !isLockPanrightPanLeft
                    moveTimeline(touch.pageX + marginLeft, 'h', $(this));
                }
            }
        }
    });
...
function eventInit() {//事件初始化
    ...
    $('.closewin img').on('touchstart', function () {
    ...
    $('.tnew').on('touchstart', function (e) {
        var event = e.originalEvent;
        for (var i = 0; i < event.targetTouches.length; i++) {
            var touch = event.targetTouches[i];
            if (touch) {
                MOUSE_POSITION_X = touch.pageX;
                MOUSE_POSITION_Y = touch.pageY;
    ...
    // 20141020 Allen上下筆資料
    $('.PrevNext').on('touchstart', function (e) {
            ...
            success: function (jData) {
                ...
                if (ajaxData['PicUrlList'].length == 0 && ajaxData['VideoUrlList'].length == 0) {
                    ...
                    if (PrevNext == 'RightPrev') {
                        allowPrevNext = true;
                        $('#RightPrev').trigger('touchstart');
                    } else if (PrevNext == 'RightNext') {
                        allowPrevNext = true;
                        $('#RightNext').trigger('touchstart');
                    } else if (PrevNext == 'LeftPrev') {
                        allowPrevNext = true;
                        $('#LeftPrev').trigger('touchstart');
                    } else if (PrevNext == 'LeftNext') {
                        allowPrevNext = true;
                        $('#LeftNext').trigger('touchstart');
                    }
